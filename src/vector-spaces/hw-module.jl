...
weight_type(::HighestWeightModule{F, A, WeightVector{W}}) where {F, A, W} = W

# Weyl dimension formula
function weyl_dim(λ::Weight, G::AbstractGroup{Lie})
    Δ⁺ = positive_roots(algebra(G))
    ρ = sum(Δ⁺) / 2
...
function orbit(
    ...
    isapprox(wv, zero(T); atol=tol) && return T[]
    push!(processed_weights, weight(wv))
    orbit_vectors = [wv]
    for nre in negative_root_elements(algebra(group(action)))
        if weight(wv) + root(nre) ∉ processed_weights
            new_wv = act(nre, wv, action)
